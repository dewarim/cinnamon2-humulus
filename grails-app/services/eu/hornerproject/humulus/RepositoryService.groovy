...
class RepositoryService {
     ...
     */
    void acquireLock(User user, ObjectSystemData osd, String repositoryName) {
     ...
     *
     */
    void unlockOsd(User user, ObjectSystemData osd, String repositoryName) {
        Client client = getClient(user.name, repositoryName)
        try {
    ...
    Boolean checkSuperuser(User user) {
        Group superusers = Group.findByName(Constants.GROUP_SUPERUSERS)
        GroupUser gu = GroupUser.findByGroupAndUser(superusers, user)
        return gu != null
    }
     ...
     * @param repositoryName name of the repository where the OSD resides
     */
    void setMetadata(User user, String id, String metadata, String repositoryName) {
     ...
     */
    String getTextContent(User user, ObjectSystemData osd, String repositoryName) {
        Client client = getClient(user.name, repositoryName)
     ...
     * granted permissions.
     */
    Set<String> getUserPermissions(User user, Acl acl, String repositoryName) {
        Client client = getClient(user.name, repositoryName)
     ...
     */
    Node getUserDetails(User user, String repositoryName) {
        Client client = getClient(user.name, repositoryName)
     ...
     * @return a set of ids
     */
    Set<Long> getFoldersInside(User user, Folder parentFolder, String repositoryName) {
        Client client = getClient(user.name, repositoryName)
     ...
     * @return
     */
    Boolean mayBrowseFolder(User user, Folder folder, String repositoryName) {
        Client client = getClient(user.name, repositoryName)
        try {
     ...
     * @return
     */
    Boolean mayBrowseOsd(User user, ObjectSystemData osd, String repositoryName) {
        Client client = getClient(user.name, repositoryName)
        try {
    ...
    List<ObjectSystemData> getObjects(User user, Folder parent, String repositoryName, String versions){
        Client client = getClient(user.name, repositoryName)
...
//            log.debug("idSet: ${idSet}")
    ...
    List<Folder> getFolders(User user, Folder parent, String repositoryName){
    ...
    void removeUserFromCache(User user, String repositoryName) {
        try {
            Client client = getClient(user?.name, repositoryName)
    ...
    Boolean checkPermission(User user, Acl acl, String repositoryName, String permissionName){
